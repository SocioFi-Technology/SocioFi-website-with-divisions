...
export default function ServicesCMSPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: pric }, { data: faq }] = await Promise.all([
      client.from('cms_pricing').select('*').eq('division', 'services').order('price'),
      client.from('cms_faqs').select('*').eq('division', 'services').order('order_index'),
    ])
  ...
  async function savePricing(item: Partial<CMSPricing>) {
    ...
    const payload = { ...item, division: 'services', updated_at: new Date().toISOString() }
    ...
    <CMSShell>
      <div style={{ paddingBottom: 48 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
          <div>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: ACCENT, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 6 }}>Services</p>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.02em', margin: 0 }}>Services Content</h1>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.95rem', color: 'var(--text-secondary)', marginTop: 6 }}>Pricing plans and frequently asked questions</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border)', marginBottom: 24 }}>
          {(['pricing', 'faqs'] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ background: 'none', border: 'none', borderBottom: tab === t ? `2px solid ${ACCENT}` : '2px solid transparent', color: tab === t ? 'var(--text-primary)' : 'var(--text-muted)', fontFamily: 'var(--font-display)', fontSize: '0.9rem', fontWeight: tab === t ? 600 : 400, padding: '10px 20px', cursor: 'pointer', marginBottom: -1, textTransform: 'capitalize' }}>
              {t === 'pricing' ? `Pricing Plans (${pricing.length})` : `FAQs (${faqs.length})`}
            </button>
          ))}
        </div>
        ...
        {tab === 'pricing' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
            {loading ? [1,2,3].map(i => <div key={i} style={{ height: 200, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }} />) :
              pricing.length === 0 ? <div style={{ gridColumn: '1/-1', padding: '48px 0', textAlign: 'center', color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>No pricing plans yet.</div> :
              pricing.map(p => (
                <div key={p.id} style={{ background: 'var(--bg-card)', border: p.is_featured ? `2px solid ${ACCENT}` : '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '22px 24px', position: 'relative' }}>
                  {p.is_featured && <div style={{ position: 'absolute', top: -10, left: 16, background: ACCENT, color: 'white', borderRadius: 100, padding: '2px 12px', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', fontWeight: 600 }}>FEATURED</div>}
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '28px 32px', width: '100%', maxWidth: 560, maxHeight: '85vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>
                  {editItem.id ? 'Edit' : 'New'} {tab === 'pricing' ? 'Pricing Plan' : 'FAQ'}
                </h2>
...
function PricingForm({ item, onSave, onCancel, inputStyle, labelStyle, accent }: { item: Partial<CMSPricing>; onSave: (i: Partial<CMSPricing>) => void; onCancel: () => void; inputStyle: React.CSSProperties; labelStyle: React.CSSProperties; accent: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div><label style={labelStyle}>Features (one per line)</label><textarea style={{ ...inputStyle, resize: 'vertical' } as React.CSSProperties} rows={5} value={featuresText} onChange={e => { setFeaturesText(e.target.value); setForm(p => ({ ...p, features: e.target.value.split('\n').filter(Boolean) })) }} /></div>
