  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px', position: 'relative' }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', fontWeight: 500, color: ACCENT, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
              <span aria-hidden="true" style={{ width: 20, height: 1.5, background: ACCENT, display: 'inline-block' }} />
              From Ventures
            </div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem, 3.5vw, 2.8rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.03em', color: 'var(--text-primary)', margin: '0 0 14px' }}>
              Founder Perspectives &amp; Startup Insights
            </h1>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', lineHeight: 1.7, color: 'var(--text-secondary)', margin: 0, maxWidth: '52ch' }}>
              Thinking on building AI-native startups, what we look for in founders, and
              what we have learned from our portfolio companies.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          {filteredPosts.length > 0 ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
              {filteredPosts.map((post, i) => (
                <motion.div key={post.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5, delay: i * 0.06, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <p style={{ fontFamily: 'var(--font-body)', color: 'var(--text-muted)', textAlign: 'center', padding: '60px 0' }}>
              No Ventures articles published yet. Check the main blog for related content.
            </p>
          )}
      ...
      <section style={{ padding: '60px 0 80px', background: 'var(--bg-2)', borderTop: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
          <BlogNewsletter accentColor={ACCENT} title="Founder insights, delivered monthly." subtitle="Thinking on AI-native startups, venture building, and what we learn from working with early-stage founders." />
        </div>
      </section>
    </>
  );
