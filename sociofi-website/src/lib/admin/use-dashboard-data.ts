  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('pipeline_entries').select('*', { count: 'exact', head: true }).eq('division', 'studio').in('stage', ['in_progress', 'review']),
        supabase.from('pipeline_entries').select('*', { count: 'exact', head: true }).eq('division', 'studio').in('stage', ['in_progress', 'review']).lt('created_at', sevenDaysAgo),
        supabase.from('agent_deployments').select('*', { count: 'exact', head: true }).eq('enabled', true),
        ...
        supabase.from('ventures_applications').select('*', { count: 'exact', head: true }).eq('status', 'screening'),
